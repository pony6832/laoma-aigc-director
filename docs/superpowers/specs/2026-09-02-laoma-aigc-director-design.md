...
新 Agent 使用「版本化知識快照」，從現有 AI 影音學習導師選取已核准方法：
...
`source-manifest.md` 記錄每項方法的來源、同步日期及適用版本。新 Agent 不直接依賴另一個技能資料夾的相對路徑，避免來源技能更新或搬移後失效。
...
原始輸入不覆蓋；衍生檔案使用明確版本號。`PROJECT_STATE.json` 記錄當前 Gate、已鎖定版本、未決事項、資產狀態與下一步。
...
驗證重點是 Agent 是否做出正確決策與建立可用產物，不測固定措辭。
