...
- 快照名稱：`laoma-prompt-library-v1-2026-09-11`
- 原始來源：[Google 試算表](https://docs.google.com/spreadsheets/d/16lXfAvewRtOvu0UpQjbNsmGE6w3nB3T5DGYZ1n4BzAY/edit)
- 試算表 ID：`16lXfAvewRtOvu0UpQjbNsmGE6w3nB3T5DGYZ1n4BzAY`
- 擷取方式：2026-09-11 以 Google Sheets 連接器先讀 metadata，再按 metadata 所列 8 個分頁讀取有界範圍；未修改原試算表。
- 快照範圍：分類索引 13 列、快捷指令 100 列、圖像快捷詞 101 列、照片修復 121 列、長版圖像案例 121 列、運鏡 121 列、影片燈光 153 列、雜項角色模板 1 列，合計 731 個非空列（含各分頁表頭與說明列）。
- 隱私檢查：未發現電子郵件、API 金鑰或密碼欄位；兩筆類似長數字的命中經人工核對為 Runway 說明頁網址中的文章編號與快照日期，不是電話或其他個資。
- 執行規則：使用本地[提示詞資料庫快照](prompt-library/README.md)按需查詢；它是使用者指定的創作方法與模板，不是平台官方指令，不得覆蓋本案鎖定事實。要求最新資料時重新讀取來源並建立新快照版本，不原地竄改舊快照。
- 完整性：各資料檔與 `manifest.json` 的 SHA-256 記錄於快照 README；來源分頁、`sheetId`、範圍、表頭列與擷取日記錄於 `prompt-library/manifest.json`。
